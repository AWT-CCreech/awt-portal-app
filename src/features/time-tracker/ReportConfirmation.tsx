...
const ReportConfirmation: React.FC<IProps> = ({
    openConfirmation, setOpenConfirmation, report, setReport, superviseeOptions, previousPeriod, setTrackersInPeriod, setSelectedSuperviseeId
}) => {
    ...
    const handleConfirm = () => {
        ...
        if (report.length > 0) {
            var body = "<html><body><h3>Total hours from %%STARTDATE%% to %%ENDDATE%%:</h3>";
            body += "<table style='width: 20%' border='1'><tr><th>Name</th><th>Hours</th></tr>";
            report.forEach(r => body += `<tr><td style='text-align: center'>${r.fullName}</td>
            ...
            agent.TimeTrackers.approve({ userIds: report.map(r => parseInt(r.userId)), previousPeriods: report.map(r => r.previousPeriod) });
        }
    };
